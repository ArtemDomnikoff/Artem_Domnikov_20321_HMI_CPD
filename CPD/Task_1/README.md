...
### Пояснение
При запуске открывается окно программы. В виджет Treeview записываются все процессы. Процессы можно сортировать нажатием на столбец с необходимым критерием, при повторном нажатии сработает обратная сортировка. Кнопка "Terminate process" закрывает процесс и удаляет его из таблицы. В поле Entry вводится имя процесса, который мы хотим найти. При нажатии кнопки "Search" в таблице выделяется процесс с именем, введённым в поле Entry. Во вкладке Graphs отображаются графики загруженности CPU, Диска, памяти и сети. Менять частоту обновления процессов можно с помощью кнопок RadioButtons внизу окна.
